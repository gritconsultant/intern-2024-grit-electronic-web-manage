    ...
    >
      ...
      <div class="flex justify-end items-end h-[100px] mr-[75px]">
        <button
          ...
          class="px-6 py-3 rounded-md h-[45%] font-semibold"
          :class="{
            'bg-orange-500 hover:bg-orange-600': isDataChanged,
            'bg-gray-400 cursor-not-allowed': !isDataChanged,
          }"
        >
          บันทึกข้อมูล
        </button>
...
const updateBank = async () => {
  await service.order
    ...
    .then((resp: any) => {
      ...
      bankRes.value = temp; // อัปเดต bankRes หลังจากบันทึกสำเร็จ
    })
    .catch((error: any) => {
